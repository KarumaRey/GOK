...
static unique_ptr<IRBuilder<>> builder; //создает объекты промежуточного представления
static map<string, AllocaInst *> named_values;
// по имени обращаться к выделенной памяти можно
...
//Abstract Syntax Tree(AST)
 ...
 //всё дальше наследуется от exprAST
class ExprAST {
public:
  virtual ~ExprAST() = default;
// объявление виртуального деструктора в базовом классе 
// если объявим как виртуальный, то будет правильное удаление объектов производных классов через указатель на базовый класс

// нет необходимости в явной реализации в базовом классе. 
//полагаемся на автоматически сгненерированный деструктоор по умолчанию
...
class FunctionAST { //узел отвечающйи за объявление функций в дереве
public:
...
/// Code generation
// создает входной блок в функцию. имя выделенного участка памяти
...
//метод работает рекурсивно. обход в глубину. просто для каждого элемента генерится codegen
